...
template <std::size_t NumWork, std::size_t Version> struct topo_work_base {
  template <class Vec> static auto get(const Vec &rhs) {
...
protected:
  ...
  static auto make_states(T &t, std::index_sequence<Index...>) {
    return std::make_tuple(
        topo_solver_state<
            std::remove_reference_t<std::tuple_element_t<Index, T>>, NumWork,
            Version, Index>::get_work(std::get<Index>(t))...);
  }
  ...
  static std::array<MV, NumWork> make(const MV &, T &wv,
                                      std::index_sequence<Index...>) {
