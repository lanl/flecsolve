...
#include "discrete_operators/common/operator_base.hh"
#include "discrete_operators/common/state_store.hh"
#include "discrete_operators/operator_task.hh"
#include "discrete_operators/operator_utils.hh"
...
template<auto Var, class Topo, class Scalar, template<class> class MixinSet>
struct operator_traits<volume_diffusion_op<Var, Topo, Scalar, MixinSet> >
{
...
template<auto Var, class Topo, class Scalar, template<class> class MixinSet>
struct volume_diffusion_op
    : operator_host<volume_diffusion_op<Var, Topo, Scalar, MixinSet>, MixinSet>
{
  // using base_type =
  //     operator_host<volume_diffusion_op<Var, Topo, T, Mixins...>, Mixins...>;
  using base_type =
      operator_host<volume_diffusion_op<Var, Topo, Scalar, MixinSet>, MixinSet>;
  ...
  // Due to flecsi restrictions, "owned" data fields need to be declared
  // as global (as explained to me, at "file-scope"). These types represent
  // these fields.
  ...
  volume_diffusion_op(topo_slot_t& s, param_type parameters)
      : base_type(parameters),
        fluxes(flux_store_t::get_state(s)),
  ...
  void _apply(topo_slot_t& m, cell_ref u, cell_ref v) const
  {
    ...
    execute<tasks_f::diffuse_op>(m, this->params.beta, this->params.alpha,
                                 *(this->params.a), u, du, v);
  }

  template<auto... Axis>
  constexpr void sweep(topo_slot_t& m,
                       cell_ref u,
                       util::constants<Axis...>) const
  {
    (execute<tasks_f::template flux_op<Axis> >(m, u, *(this->params.b),
                                               fluxes[Axis]),
     ...);
