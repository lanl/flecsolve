...
struct operator_mesh : topo::specialization<topo::narray, operator_mesh> {
  ...
  template <class B> struct interface : B {
    ...
    /**
     * @brief returns a tuple of an index iterator that is continuous along the
     * specified axis, and a set of offset values based on the requested stencil
     * values. "stenils" here refers to a set of integer literals.
     *
     ...
     * @tparam II integer sequence of "stencil" values along axis A. e.g. {-1,
     * 1} for left, right
    ...
    decltype(auto) get_stencil(std::integer_sequence<int, II...>) {
      ...
      // we want to step through axis Along, so rotate strides so that these
      // indicies are "continuous" in that direction NOTE: we do this based on
      // how the indicies are calculated, see make_subrange_ids
      ...
      // get<N > 0>: the offset values for the stenil.
        ...
        return std::make_tuple(utils::make_subrange_ids<To, axes::size>(sr, st),
  ...
  protected:
    ...
    utils::srange grid_subrange() {
      if constexpr (Along != A) {
        ...
        const std::size_t end = B::template logical<index_space::cells, A, 1>();
        return {start, end};
      } else {
        if constexpr (DM == logical) {
          ...
          const std::size_t end = B::template size<Space, A, __dm<DM>()>();
          return {start, end};
        } else if constexpr (DM == boundary_high) {
          const std::size_t start = B::template logical<Space, A, 1>();
