...
// types.
// *********************
template <auto... V> using has = flecsi::util::constants<V...>;

template <auto A, class B> struct cons_ {};
template <auto A0, auto... A> struct cons_<A0, has<A...>> {
  using type = has<A0, A...>;
};
template <auto A, class B> using cons = typename cons_<A, B>::type;
...
// end of T.
template <class S, class SS> struct complement_list_<S, nil, SS> {
  using type = nil;
};
...
template <class S> struct rotate_;
template <class S> using rotate = typename rotate_<S>::type;

template <auto S, auto... T> struct rotate_<has<S, T...>> {
  using type = has<T..., S>;
};

template <auto T, class S> struct rotate_to_;

template <auto T, class S> using rotate_to = typename rotate_to_<T, S>::type;

template <auto S, auto... T> struct rotate_to_<S, has<S, T...>> {
  using type = has<S, T...>;
};

template <auto S, auto P, auto... T> struct rotate_to_<S, has<P, T...>> {
...
inline flecsi::util::id translate(flecsi::util::id &x, std::size_t stride,
                                  const srange &sub) {
  flecsi::util::id ret;
...
template <int... OFFSETS> constexpr auto offset_seq() {
