...
template <class Derived> struct operator_traits;
template <class Derived> struct operator_parameters;

template <class Derived, template <class> class BaseType> struct operator_base {
...
struct operator_settings : operator_base<Derived, operator_settings> {
  ...
  template <class P> operator_settings(P &&p) : base_type() { this->reset(p); }

  template <class... Ps> constexpr static param_type getParamType(Ps &&...ps) {
    return param_type{ps...};
  }

  template <class P> constexpr decltype(auto) reset(P &&pars) {
...
template <template <class> class... Mixins> struct make_mixins {
  template <class Derived> struct templ : Mixins<Derived>... {
...
template <class ML> struct execute_mixins;
