...
template <class T> struct OpExpr;

template <class... Ps> struct OpExpr<std::tuple<Ps...>> {
  std::tuple<Ps...> ops;

  constexpr OpExpr(Ps... ps) : ops(std::forward<Ps>(ps)...) {}

  template <class U, class V> constexpr void apply(U &&u, V &&v) const {
    std::apply(
        [&](auto &&...a) {
...
template <class... Ps> inline constexpr auto op_expr(Ps &&...ps) {
