...
struct mesh {
	...
	void copy(const Other & x, vec_data & z) {
		...
		execute<tasks::template copy<typename Other::template acc_all<ro>>>(
			x.topo, z.ref(), x.ref());
	}
	...
	void subtract(const vec_data & x, const vec_data & y, vec_data & z) {
		if (x.fid() == z.fid()) {
			execute<tasks::template subtract_self<true>>(
				z.topo, z.ref(), y.ref());
		}
		else if (y.fid() == z.fid()) {
			execute<tasks::template subtract_self<false>>(
				z.topo, z.ref(), x.ref());
		}
		else {
	...
	void divide(const vec_data & x, const vec_data & y, vec_data & z) {
		if (z.fid() == x.fid()) {
			execute<tasks::template divide_self<true>>(
				z.topo, z.ref(), y.ref());
		}
		else if (z.fid() == y.fid()) {
			execute<tasks::template divide_self<false>>(
				z.topo, z.ref(), x.ref());
		}
		else {
	...
	void linear_sum(scalar alpha,
	                ...
	                vec_data & z) {
		if (z.fid() == x.fid()) {
			execute<tasks::template linear_sum_self<true>>(
				z.topo, z.ref(), y.ref(), alpha, beta);
		}
		else if (z.fid() == y.fid()) {
			execute<tasks::template linear_sum_self<false>>(
				z.topo, x.ref(), z.ref(), alpha, beta);
		}
		else {
			execute<tasks::linear_sum>(
	...
	axpy(scalar alpha, const vec_data & x, const vec_data & y, vec_data & z) {
		if (z.fid() == x.fid()) {
			execute<tasks::template axpy_self<true>>(
				z.topo, z.ref(), y.ref(), alpha);
		}
		else if (z.fid() == y.fid()) {
			execute<tasks::template axpy_self<false>>(
	...
	void axpby(scalar alpha, scalar beta, const vec_data & x, vec_data & z) {
		execute<tasks::axpby>(z.topo, z.ref(), x.ref(), alpha, beta);
	}
	...
	auto min(const vec_data & x) const {
		return reduce<tasks::local_min, flecsi::exec::fold::min>(x.topo,
		                                                         x.ref());
	}

	auto max(const vec_data & y) const {
		return reduce<tasks::local_max, flecsi::exec::fold::max>(y.topo,
		                                                         y.ref());
	}
	...
	auto lp_norm_local(const vec_data & x) const {
		if constexpr (p == 1) {
			return reduce<tasks::l1_norm_local, exec::fold::sum>(x.topo,
			                                                     x.ref());
		}
		else if constexpr (p == 2) {
			return reduce<tasks::l2_norm_local, exec::fold::sum>(x.topo,
			                                                     x.ref());
		}
		else {
			return reduce<tasks::lp_norm_local, exec::fold::sum>(
					   x.topo, x.ref(), p)
			    .get();
		}
	}
	...
	auto dot(const vec_data & x, const vec_data & y) const {
		return reduce<tasks::scalar_prod, exec::fold::sum>(
			x.topo, x.ref(), y.ref());
	}
