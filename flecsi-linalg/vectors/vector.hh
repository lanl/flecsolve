...
public:
  ...
  template <class D> vector(D &&d) : data(std::forward<D>(d)) {}
   ...
   */
  template <class Other> void copy(const Other &other) {
   ...
   * \param[in] y vector
   */
  void divide(const vec &x, const vec &y) { ops.divide(x.data, y.data, data); }
   ...
   */
  template <unsigned short p> auto lp_norm() const {
  ...
  template <auto var> constexpr decltype(auto) subset(variable_t<var>) {
    static_assert(var == Variable);
    return *this;
  }

  template <auto var> constexpr decltype(auto) subset(multivariable_t<var>) {
    static_assert(var == Variable);
    return *this;
  }
