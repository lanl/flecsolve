...
struct solver_factory : with_solver_erasure, with_label {
	...
	std::shared_ptr<solver_parameters> get_parameters() {
		return parameters;
	}
...
struct krylov_factory : solver_factory<krylov_factory> {
	...
	void set_solver_type(registry reg) {
		solver_type = reg;
	}
	...
	void create_parameters(registry reg, V & v, Args &&... args) {
		switch (reg) {
		case registry::cg: {
			parameters = make_params<con_types<registry::cg>>(v, std::forward<Args>(args)...);
			break;
		}
		case registry::gmres: {
			parameters = make_params<con_types<registry::gmres>>(v, std::forward<Args>(args)...);
			break;
		}
		case registry::bicgstab: {
			parameters = make_params<con_types<registry::bicgstab>>(v, std::forward<Args>(args)...);
		}}
	}
	...
	std::unique_ptr<storage> create(V & v, Args&&... args) {
		...
		switch (solver_type) {
		case registry::cg : {
			ret = make_storage<con_types<registry::cg>>(v, std::forward<Args>(args)...);
			break;
		}
		case registry::gmres : {
			ret = make_storage<con_types<registry::gmres>>(v, std::forward<Args>(args)...);
			break;
		}
		case registry::bicgstab : {
			ret = make_storage<con_types<registry::bicgstab>>(v, std::forward<Args>(args)...);
		}
		}
	...
	std::unique_ptr<solver> wrap(storage * store, V & v, Args&&... args) {
		...
		switch (solver_type) {
		case registry::cg : {
			ret = make_wrapper<con_types<registry::cg>, D, R>(store, v, std::forward<Args>(args)...);
			break;
		}
		case registry::gmres : {
			ret = make_wrapper<con_types<registry::cg>, D, R>(store, v, std::forward<Args>(args)...);
		...
		case registry::bicgstab : {
			ret = make_wrapper<con_types<registry::bicgstab>, D, R>(store, v, std::forward<Args>(args)...);
		}
		}
...
protected:
	...
	auto make_params(V & v, Args && ... args) {
		op::krylov_parameters params(typename ctypes::settings(label("parameters").c_str()),
		                             ctypes::workgen::get(v),
		                             std::forward<Args>(args)...);
		return std::make_shared<solver_param_wrapper<decltype(params)>>(std::move(params));
	}

	template<class ctypes, class V, class ... Args>
	auto make_storage(V & v, Args && ... args) {
		auto * params = dynamic_cast<
			solver_param_wrapper<
	...
	auto make_wrapper(storage * store, V & v, Args && ... args) {
		...
		auto * slv = dynamic_cast<
			storage_wrapper<
				decltype(op::krylov(params->target))>*>(store);
		assert(slv);

		return std::make_unique<solver_wrapper<D, R,
			typename std::remove_reference_t<decltype(*slv)>::type>>(slv->target);
	}

	registry solver_type;
};

inline std::istream & operator>>(std::istream & in, krylov_factory::registry & reg) {
	std::string tok;
	in >> tok;

	if (tok == "cg") reg = krylov_factory::registry::cg;
	else if (tok == "gmres") reg = krylov_factory::registry::gmres;
	else if (tok == "bicgstab") reg = krylov_factory::registry::bicgstab;
...
std::istream & operator>>(std::istream & in, union_registry<R...> & ureg) {
	...
	if (allbad) in.setstate(std::ios_base::failbit);
	return in;
}
}
...
struct factory_union : solver_factory<factory_union<Facts...>>
{
	...
	void set_options_name(const std::string & name) {
		...
		std::apply([&](auto & ... fact) {
			(fact.set_options_name(name.c_str()), ...);
		}, members);
	}

	void set_solver_type(registry reg) {
		solver_type.value = reg.value;
	}

	template<class V, class ... Args>
	void create_parameters(registry reg, V & v, Args && ... args) {
		std::visit([&](auto reg_value) {
			std::apply([&](auto & ... facts) {
				([&](auto & fact) {
					using RT = std::decay_t<decltype(reg_value)>;
					if constexpr (std::is_same_v<RT,
					              typename std::decay_t<decltype(fact)>::registry>) {
						fact.create_parameters(reg_value, v,
						                       std::forward<Args>(args)...);
		...
		std::visit([&](auto reg_value) {
			std::apply([&](auto & ... facts) {
				([&](auto & fact) {
					...
					if constexpr (std::is_same_v<RT,
					              typename std::decay_t<decltype(fact)>::registry>) {
						ret = fact.create(v, std::forward<Args>(args)...);
					}
				}(facts),...);
			}, members);
		}, solver_type.value);
		...
		std::visit([&](auto reg_value) {
			std::apply([&](auto & ... facts) {
				([&](auto & fact) {
					...
					if constexpr (std::is_same_v<RT,
					              typename std::decay_t<decltype(fact)>::registry>) {
						ret = fact.template wrap<D, R>(store, v, std::forward<Args>(args)...);
					}
				}(facts),...);
			}, members);
		}, solver_type.value);
