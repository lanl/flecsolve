...
struct topo_tasks {
	...
	static scalar scalar_prod(flecsi::exec::accelerator s,
	                          topo_acc m,
	                          acc<ro> x,
	                          acc<ro> y) noexcept {
		scalar res =
			s.executor()
				.named("scalar_prod")
				.reduceall(
					dof, up, util::dofs(m), flecsi::exec::fold::sum, scalar) {
	...
	static void set_to_scalar(flecsi::exec::accelerator s,
	                          topo_acc m,
	                          acc<wo> x,
	                          scalar val) noexcept {
	...
	static void copy(flecsi::exec::accelerator s,
	                 topo_acc,
	                 acc_all<wo> xa,
	                 OtherAcc ya) noexcept {
		flecsi::util::iota_view<std::size_t> v(0, xa.span().size());
		s.executor().forall(i, v) { xa[i] = ya[i]; };
	}

	static void add_self(flecsi::exec::accelerator s,
	                     topo_acc m,
	                     acc<rw> z,
	                     acc<ro> x) noexcept {
	...
	static void add(flecsi::exec::accelerator s,
	                topo_acc m,
	                acc<wo> z,
	                acc<ro> x,
	                acc<ro> y) noexcept {
	...
	static void subtract(flecsi::exec::accelerator s,
	                     topo_acc m,
	                     acc<wo> x,
	                     acc<ro> a,
	                     acc<ro> b) noexcept {
	...
	template<bool inv>
	static void subtract_self(flecsi::exec::accelerator s,
	                          topo_acc m,
	                          acc<rw> x,
	                          acc<ro> b) noexcept {
	...
	static void multiply(flecsi::exec::accelerator s,
	                     topo_acc m,
	                     acc<wo> z,
	                     acc<ro> x,
	                     acc<ro> y) noexcept {
	...
	static void multiply_self(flecsi::exec::accelerator s,
	                          topo_acc m,
	                          acc<rw> x,
	                          acc<ro> y) noexcept {
		s.executor().forall(dof, util::dofs(m)) { x[dof] = x[dof] * y[dof]; };
	}

	template<bool inv>
	static void divide_self(flecsi::exec::accelerator s,
	                        topo_acc m,
	                        acc<rw> z,
	                        acc<ro> x) noexcept {
		s.executor().forall(dof, util::dofs(m)) {
	...
	static void divide(flecsi::exec::accelerator s,
	                   topo_acc m,
	                   acc<wo> z,
	                   acc<ro> x,
	                   acc<ro> y) noexcept {
	...
	static void reciprocal_self(flecsi::exec::accelerator s,
	                            topo_acc m,
	                            acc<rw> x) noexcept {
		s.executor().forall(dof, util::dofs(m)) { x[dof] = 1.0 / x[dof]; };
	}

	static void reciprocal(flecsi::exec::accelerator s,
	                       topo_acc m,
	                       acc<wo> x,
	                       acc<ro> y) noexcept {
	...
	template<class F, class T>
	static void axpy_future(flecsi::exec::accelerator s,
	                        topo_acc m,
	                        acc<wo> z,
	                        future<T> alpha_fut,
	                        F f,
	                        acc<ro> x,
	                        acc<ro> y) noexcept {
		scalar alpha = f(alpha_fut.get());
		axpy(s, m, z, alpha, x, y);
	}

	template<bool inv>
	static void axpy_self(flecsi::exec::accelerator s,
	                      topo_acc m,
	                      acc<rw> z,
	                      acc<ro> x,
	...
	template<bool inv, class F, class T>
	static void axpy_self_future(flecsi::exec::accelerator s,
	                             topo_acc m,
	                             acc<rw> z,
	                             acc<ro> x,
	                             future<T> alpha_fut,
	                             F f) noexcept {
		scalar alpha = f(alpha_fut.get());
		axpy_self<inv>(s, m, z, x, alpha);
	}

	static void axpby(flecsi::exec::accelerator s,
	                  topo_acc m,
	                  acc<rw> y,
	                  acc<ro> x,
	                  scalar alpha,
	...
	static void abs(flecsi::exec::accelerator s,
	                topo_acc m,
	                acc<wo> y,
	                acc<ro> x) noexcept {
	...
	static void add_scalar_self(flecsi::exec::accelerator s,
	                            topo_acc m,
	                            acc<rw> x,
	                            scalar alpha) noexcept {
	...
	static real
	l1_norm_local(flecsi::exec::accelerator s, topo_acc m, acc<ro> u) noexcept {
		real ret = s.executor().reduceall(
			dof, up, util::dofs(m), flecsi::exec::fold::sum, real) {
			up(std::abs(u[dof]));
		};
	...
	static real
	l2_norm_local(flecsi::exec::accelerator s, topo_acc m, acc<ro> u) noexcept {
	...
	static real
	local_max(flecsi::exec::accelerator s, topo_acc m, acc<ro> u) noexcept {
		if constexpr (is_complex) {
	...
	static real
	local_min(flecsi::exec::accelerator s, topo_acc m, acc<ro> u) noexcept {
		if constexpr (is_complex) {
			auto ret = s.executor().reduceall(
				dof, up, util::dofs(m), flecsi::exec::fold::min, real) {
				up(u[dof].real());
			};
			return ret;
		}
	...
	static real inf_norm_local(flecsi::exec::accelerator s,
	                           topo_acc m,
	                           acc<ro> x) noexcept {
		auto ret = s.executor().reduceall(
			dof, up, util::dofs(m), flecsi::exec::fold::max, real) {
	...
	static void dump(flecsi::exec::accelerator s,
	                 std::string_view pre,
	                 topo_acc m,
	                 acc<ro> x) {
