...
auto add = std::make_pair([](double gid) {
					 return std::make_pair(rconv<0>(gid) + rconv<2>(gid),
	                                       cconv<0>(gid) + cconv<2>(gid));
				 },
                 "add");

auto subtract = std::make_pair([](double gid) {
						  return std::make_pair(rconv<0>(gid) - rconv<2>(gid),
	                                            cconv<0>(gid) - cconv<2>(gid));
					  },
                      "subtract");
auto mult = std::make_pair([](double gid) {
					  return std::make_pair(rconv<0>(gid) * rconv<2>(gid),
	                                        cconv<0>(gid) * cconv<2>(gid));
				  },
                  "multiply");
auto scalar_add = std::make_pair([](double gid) {
							return std::make_pair(rconv<0>(gid) + 1,
	                                              cconv<0>(gid) +
	                                                  conv((1. + 1i)));
						},
                        "add scalar");
auto divide = std::make_pair([](double gid) {
						return std::make_pair(
							rconv<1>(gid) / (rconv<0>(gid) + 1),
							cconv<1>(gid) / (cconv<0>(gid) + conv((1. + 1i))));
					},
                    "divide");
auto scale = std::make_pair([](double gid) {
					   return std::make_pair(rconv<0>(gid) * 2,
	                                         cconv<0>(gid) * 2.4);
				   },
                   "scale");
auto recip = std::make_pair([](double gid) {
					   return std::make_pair(
						   1.0 / (rconv<1>(gid) + 1),
						   1.0 / (cconv<1>(gid) + conv((1. + 1i))));
				   },
                   "reciprocal");
auto linsum = std::make_pair([](double gid) {
						return std::make_pair(
							rconv<1>(gid) * 8 + rconv<2>(gid) * 9,
							cconv<1>(gid) * 8. + cconv<2>(gid) * 9.);
					},
                    "linear sum");
auto axpy = std::make_pair([](double gid) {
					  return std::make_pair(rconv<0>(gid) * 7 + rconv<1>(gid),
	                                        cconv<0>(gid) * conv((7. + 3i)) +
	                                            cconv<1>(gid));
				  },
                  "axpy");
auto axpby = std::make_pair([](double gid) {
					   return std::make_pair(
                   ...
                   "axpby");
auto abs = std::make_pair([](double gid) {
					 return std::make_pair(
