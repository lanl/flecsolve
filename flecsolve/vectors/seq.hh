...
struct seq : base<Derived> {
	...
	template<class D>
	seq(D && d) :
		base_t{std::forward<D>(d)} {}

	constexpr scalar & operator[](len_t i) {
		return data[i];
	}

	constexpr const scalar & operator[](len_t i) const {
		return data[i];
	}
};
...
struct seq_view : seq<seq_view<T>> {
	...
	seq_view(flecsi::util::span<T> span) :
		base{span} {}
};
...
struct seq_work {
	...
	seq_vec<T, var> & get() {
		...
		if (vecs[I].data.size() != csize) vecs[I].data.resize(csize);
