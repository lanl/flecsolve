...
std::istream & operator>>(std::istream & in, predictor & pred) {
	...
	if (token == "ab2") pred = predictor::ab2;
	else if (token == "leapfrog") pred = predictor::leapfrog;
...
std::istream & operator>>(std::istream & in, method & meth) {
	...
	if (token == "BE")        meth = method::be;
	else if (token == "CN")   meth = method::cn;
	else if (token == "BDF2") meth = method::bdf2;
	else if (token == "BDF3") meth = method::bdf3;
	else if (token == "BDF4") meth = method::bdf4;
	else if (token == "BDF5") meth = method::bdf5;
	else if (token == "BDF6") meth = method::bdf6;
	else in.setstate(std::ios_base::failbit);
...
std::istream & operator>>(std::istream & in, controller & cont) {
	...
	if (token == "PC.4.7")        cont = controller::pc4_7;
	else if (token == "H211b")    cont = controller::H211b;
	else if (token == "PC11")     cont = controller::pc11;
	else if (token == "Deadbeat") cont = controller::deadbeat;
	else in.setstate(std::ios_base::failbit);
...
std::istream & operator>>(std::istream & in, error_scaling & s) {
	...
	if (tok == "fixed-scaling")         s = error_scaling::fixed_scaling;
	else if (tok == "fixed-resolution") s = error_scaling::fixed_resolution;
	else in.setstate(std::ios_base::failbit);
