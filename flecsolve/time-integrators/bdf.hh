...
struct parameters : time_integrator::parameters<Op, Work> {
	...
	void validate() {
		flog_assert(memory_size(starting_integrator) == 1, "Starting integrator must be CN or BE");
		...
		if (calculate_time_trunc_error) {
			...
			if (time_error_scaling == error_scaling::fixed_scaling) {
				flog_assert(problem_scales.size() > 0, "Problem scales must be specified if using fixed time error scaling");
			}
		}

		if (use_predictor) {
			if (memory_size(integrator) > 1) {
				flog_assert(predictor == bdf::predictor::leapfrog, "Only the leapfrog predictor is supported for BDF2-6");
			} else if (integrator == method::cn) {
				flog_assert(predictor == bdf::predictor::ab2, "Valid option for Crank-Nicolson predictor is only ab2 currently");
			}
		}
	}
...
struct integrator : base<parameters<O, W, S>> {
	...
	integrator(P p) : base<P>(std::move(p)),
	                  prev(memory_size(params.integrator),
	                       std::ref(params.work)),
	...
	void advance(double dt, bool first_step, Curr & curr, Out & out) {
		flog_assert(curr != out, "BDF integrator: curr cannot be the same as out");
		flog_assert(steps_remaining() && (current_time < params.final_time), "BDF integrator: already finished integrating");
	...
	bool check_solution() {
		...
		if (!check_passed) ++total_steprejects;
	...
	double get_next_dt(bool good_solution) {
		...
		if (params.timestep_strategy == strategy::truncation_error) {
			if (good_solution) {
				current_dt = estimate_dt_with_truncation_error_estimates(current_dt, good_solution);
			...
			if (good_solution) {
				if (params.timestep_strategy == strategy::constant)
					...
					if (i < params.number_of_time_intervals) {
						current_dt = params.initial_dt + ((double) i) /
							((double) params.number_of_time_intervals) * (params.max_dt - params.initial_dt);
						++i;
					} else {
						current_dt = params.max_dt;
					}
				} else if (params.timestep_strategy == strategy::limit_relative_change) {
		...
		if (good_solution) old_dt = tmp_dt;

		current_dt = std::min(std::min(current_dt, params.max_dt), params.final_time - current_time);
		return current_dt;
	}
...
protected:
	...
	void set_initial_guess(bool first_step, double current_time, double current_dt, double old_dt)  {
		(void) current_time; (void) current_dt;	(void) old_dt;
	...
	void compute_source_term() {
		...
		// add in time dependent source, g, for problems of the form u_t = f(u)+g
		// or for MGRIT the FAS correction
		// if (params.has_source_term) {
	...
	void evaluate_predictor() {
		...
		case method::cn:
			if (params.predictor == predictor::ab2) {
				...
				flog_error("Crack-Nicolson currently only supports ab2 predictor");
			}
			break;
		case method::be:
		...
		default:
			if (first_step) {
				if (params.starting_integrator == method::cn ||
					...
					flog_error("ERROR: starting integrator must be CN or BE");
				}
			} else {
				if (params.predictor == predictor::leapfrog) {
					evaluate_leapfrog_predictor();
				} else {
					flog_error("BDF2-6 currently only supports the leapfrog predcictor");
				}
			}
		}
	...
	*/
	double estimate_dynamical_time_scale(double curr_dt) {
		...
		scratch.add_scalar(prev[0].solution, std::numeric_limits<double>::epsilon());
		...
		auto actual_relative_change = std::max_element(relative_change_in_vars.begin(), relative_change_in_vars.end());
		...
		double cfl_new_dt =
			std::sqrt(params.target_relative_change / (*actual_relative_change)) * curr_dt;
	...
	double estimate_dt_with_truncation_error_estimates(double current_dt,
		...
		*/
		...
		if (params.use_pi_controller && good_solution && (!prev_successive_rejects)) {
			...
			case controller::H211b: {
				double b = 4.;
				dt_factor = std::pow(safety_factor / std::max(time_trunc_err_est, eps),
				                     1. / (b * (p + 1.)));
				dt_factor *=
					std::pow(safety_factor / std::max(prev_time_trunc_err_est, eps),
					         1. / (b * (p + 1.)));
			...
			case controller::pc4_7: {
				dt_factor = std::pow(safety_factor / std::max(time_trunc_err_est, eps),
				                     0.4 / (p + 1.));
				dt_factor *= std::pow(time_err_est_ratio, 0.7 / (p + 1.)) * alpha;
				break;
			}
			case controller::pc11: {
				dt_factor = std::pow(safety_factor / std::max(time_trunc_err_est, eps),
				                     1.0 / (p + 1.));
				dt_factor *= std::pow(time_err_est_ratio, 1. / (p + 1.)) * alpha;
				break;
			}
			...
			/* The safety factor for the deadbet when recovering from time step rejects should be
			   very conservative */
			double safety_factor = 0.8;
			dt_factor = std::pow(safety_factor / std::max(time_trunc_err_est, eps),
			                     1. / (p + 1.));
	...
	double calculate_LTE_scaling_factor() {
		...
		if (first_step) {
			if (params.starting_integrator == method::be)
				...
				error_factor = 1.5; // fairly arbitrary factor, used because the error estimator
	...
	void calculate_scaled_LTE_norm(X & x, Y & y, std::array<double, N> & norms) {
		auto & scratch = getvec<workvec::scratch>();
		...
		case error_scaling::fixed_scaling:
			...
			std::size_t i{0};
			vec::apply([&](const auto & sol_vec, auto & scratch_vec) {
				scratch_vec.add_scalar(sol_vec, params.problem_scales[i++]);
			}, x, scratch);
		...
		std::size_t i{0};
		vec::apply([this, &i, &norms](const auto & comp) {
			if (params.time_trunc_err_norm == vec::norm_type::inf)
	...
	void calculate_temporal_truncation_error() {
		...
		constexpr auto num_components = std::remove_reference_t<decltype(sol_vec)>::num_components;
		std::array<double, num_components> trunc_err_est;
		trunc_err_est.fill(1.);

		if ((integrator_step > 0) || (first_step && params.use_initial_predictor)) {
			/*
			...
			*/
			...
			auto & y = (first_step && (params.starting_integrator == method::cn))
				? prev[0].solution : getvec<workvec::predict>();
			...
			time_trunc_err_est = *(std::max_element(trunc_err_est.begin(), trunc_err_est.end()));
			time_trunc_err_est = std::max(std::numeric_limits<double>::epsilon(), time_trunc_err_est);

			// the time error ratio is only used for a PI based controller
			time_err_est_ratio = prev_time_trunc_err_est / time_trunc_err_est;
		} else {
			// for the fist time step pretend we met the truncation error estimate
...
struct integrator<O, W, S>::history {
	...
	history(size_type mlen, std::reference_wrapper<W> work) :
		vecs(workvec::nwork, work_size - workvec::nwork),
		pos(0), len(mlen), work(work) {
