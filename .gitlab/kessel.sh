...
export FLECSOLVE_CHECKOUT=$(realpath $PARENT_DIR/..)
...
if [ "$SYSTEM_NAME" == "darwin" ] || [ "$SYSTEM_NAME" == "rocinante" ]; then
  ...
  export AMP_DATA=${AMP_DATA:-/usr/projects/xcap/spack-env/AMP-Data.tar.gz}
  ...
  export AMP_DATA=${AMP_DATA:-/usr/workspace/xcap/spack-env/AMP-Data.tar.gz}
else
